...
public class UserSkillLevelChangedEventListener extends AbstractExternalListener<UserSkillLevelChangedEvent> {
    ...
    public void onUserSkillLevelChanged(@RequestBody CloudEvent<UserSkillLevelChangedEvent> cloudEvent, @RequestHeader Map<String, String> headers) {
        super.handle(cloudEvent, UserSkillLevelChangedEventListener::getContext, headers);
    }

    @Override
    protected PersistentUserSkillLevelChangedEvent mapToPersistentEvent(UserSkillLevelChangedEvent event) {
        log.info("UserSkillLevelChangedEvent: {}", event);
        final PersistentUserSkillLevelChangedEvent persistentEvent = new PersistentUserSkillLevelChangedEvent();
        persistentEvent.setUserId(event.getUserId());
        persistentEvent.setSkillId(event.getSkillId());
        persistentEvent.setBloomLevel(event.getBloomLevel());
        persistentEvent.setOldValue(event.getOldValue());
        persistentEvent.setNewValue(event.getNewValue());
