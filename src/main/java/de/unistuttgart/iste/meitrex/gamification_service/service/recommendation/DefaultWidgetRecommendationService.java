    ...
    public List<GamificationCategoryRecommendation> getUserWidgetRecommendations(@NotNull UUID userId, @NotNull UUID courseId) {
        ...
        if(recommendationsEntity.isEmpty() || isNewWidgetRecommendationDue(recommendationsEntity.get(), settings)) {
            ...
            var recommendations = IntStream.range(0, settings.getNumberOfRecommendations())
                    .mapToObj(i -> recommendationCreator.makeRecommendation(userId, courseId, RecommendationType.WIDGET))
                    .toList();
        ...
        return recommendationsEntity.get().getRecommendations().stream().map(x -> new GamificationCategoryRecommendation(x, feedbackRequestDeadline.isFeedbackRequestDue(userId, x))).toList();
    }
